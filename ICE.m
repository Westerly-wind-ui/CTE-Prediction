clc;clear;close all;	
load('R_21_Oct_2024_17_33_52.mat')	
random_seed=G_out_data.random_seed
rng(random_seed)  %固定随机数种子 	
	
data_str="F:\处理中\软化温度模型构建\Database（首列为序号，必须有）.xlsx" ;    %读取数据的路径 	
...
max_epoch=G_out_data.max_epoch1;   %maxepoch	fit_fitctreebag
...
X_all = X_new_DATA(:,1:end-1);
x_all_mu = mean(X_all);  x_all_sig = std(X_all);
X_all_norm = X_all.*x_all_sig+x_all_mu;
...
t1=clock; 	
num_tree=100;   %集成树的棵树	
[Mdl,fitness,Convergence_curve] = optimize_fitrtreebag1(train_x_feature_label_norm,train_y_feature_label_norm,vaild_x_feature_label_norm,vaild_y_feature_label_norm,num_pop,num_iter,method_mti);   	
...
% shap_interaction_values = compute_shap_interaction_values(Mdl,X_all_norm);
...
%反标准化之前的操作
 %y_train_predict_norm_values = zeros(size(y_train_predict_norm));
 %for i = 1:length(y_train_predict_norm)  
 %   y_train_predict_norm_values(i) = str2double(y_train_predict_norm{i});  
 %end
 %y_vaild_predict_norm_values = zeros(size(y_vaild_predict_norm));
 %for i = 1:length(y_vaild_predict_norm)  
 %   y_vaild_predict_norm_values(i) = str2double(y_vaild_predict_norm{i});  
 %end
 %y_test_predict_norm_values = zeros(size(y_test_predict_norm));
 %for i = 1:length(y_test_predict_norm)  
 %   y_test_predict_norm_values(i) = str2double(y_test_predict_norm{i});  
 %end
...
% 	
%     Mdlkf=TreeBagger(Mdl.NumTrees,x_feature_label_norm_all_traink,y_feature_label_norm_all_traink,'Method','regression','MinLeafSize',Mdl.MinLeafSize);	
%     %shap_values = calculate_shap_k(k,x_feature_label_norm_all,Mdlkf);
%     %shap_interaction_values = compute_shap_interaction_values_k(k,Mdlkf,x_feature_label_norm_all);
%     shap_values = calculate_shap_k(k,x_feature_label_norm_all_traink,Mdlkf);
%     shap_interaction_values = compute_shap_interaction_values_k(k,Mdlkf,x_feature_label_norm_all_traink);
% 
%     
%     %matlab代码训练
%     Mdl_kfold{1,k}=Mdlkf;	
...
%     y_train_predict_norm_all_traink=predict(Mdlkf,x_feature_label_norm_all_traink);
%     y_train_predict_all_traink= y_train_predict_norm_all_traink*y_sig+y_mu;
%     y_feature_label_all_traink= y_feature_label_norm_all_traink*y_sig+y_mu;
%     
% 
%     y_feature_label_all_traink_py = py.numpy.array(y_feature_label_all_traink);
%     y_feature_label_all_traink_py_name = strcat('y_feature_label_all_traink_py_', num2str(k), '.npy');   
%     py.numpy.save(y_feature_label_all_traink_py_name, y_feature_label_all_traink_py);
% 
%     y_train_predict_all_traink_py = py.numpy.array(y_train_predict_all_traink);
%     y_train_predict_all_traink_name = strcat('y_train_predict_all_traink_py_', num2str(k), '.npy');   
%     py.numpy.save(y_train_predict_all_traink_name, y_train_predict_all_traink_py);
% 
%     y_test_predict_all_testk_py = py.numpy.array(y_test_predict_all_testk);
%     y_test_predict_all_testk_name = strcat('y_test_predict_all_testk_py_', num2str(k), '.npy');   
%     py.numpy.save(y_test_predict_all_testk_name, y_test_predict_all_testk_py);
% 
%     y_feature_label_all_testk_py = py.numpy.array(y_feature_label_all_testk);
%     y_feature_label_all_testk_name = strcat('y_feature_label_all_testk_py_', num2str(k), '.npy');   
%     py.numpy.save(y_feature_label_all_testk_name, y_feature_label_all_testk_py);
%     
% end	
% 	
...
%  predict_str="Z:\pythonproject11\\结果保存.xlsx"; 	
% 	
...
% 计算该特征的部分依赖图
featureValues = linspace(min(test_x_feature_label_norm(:, featureIdx)), max(test_x_feature_label_norm(:, featureIdx)), 100); % 特征值范围
nInstances = size(test_x_feature_label_norm, 1); % 数据集的实例数
iceValues = zeros(nInstances, 100); % 用来存储每个实例的ICE曲线

% 遍历特征值并计算相应的预测值
for i = 1:100
    % 创建与测试集相同的特征数据，但固定其他特征
    tempData = test_x_feature_label_norm;
    tempData(:, featureIdx) = featureValues(i);
    
    % 使用模型预测
    iceValues(:, i) = predict(Mdl, tempData); % 存储每个实例的预测值
end
...
hold on;
for j = 1:nInstances
    plot(featureValues, iceValues(j, :), 'b-', 'LineWidth', 0.5); % 每个实例的ICE曲线
end
xlabel('AFE''s Value');


ylabel('Prediction');
title('ICE Plot for AFE');
grid on;
hold off;

% 绘制损失曲线
figure;
plot(Convergence_curve, '-o', 'LineWidth', 2, 'MarkerSize', 6);
title('Loss Curve (Convergence Curve)');
xlabel('Iteration');
ylabel('Loss');
grid on;
