...
augdata_all=[train_x_feature_label_aug,train_y_feature_label;vaild_x_feature_label_aug,vaild_y_feature_label;test_x_feature_label,test_y_feature_label];	
    ...
    %% 算法处理块
        ...
        Mdl = fitrsvm(train_x_feature_label_norm,train_y_feature_label_norm,'KernelFunction','gaussian');
        ...
        Time=t2(3)*3600*24+t2(4)*3600+t2(5)*60+t2(6)-(t1(3)*3600*24+t1(4)*3600+t1(5)*60+t1(6));
            disp('决策树回归')
            t1=clock;
            Mdl= fitrtree(train_x_feature_label_norm,train_y_feature_label_norm);
    ...
    for k = 1:Kfold_num
        ...
        Mdlkf=fitrsvm(x_feature_label_norm_all_traink,y_feature_label_norm_all_traink);%SVM
